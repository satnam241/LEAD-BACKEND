...
async function checkAndNotify() {
  try {
    ...
    const dueTodayLeads = await Lead.find({
      isDeleted: false,
      "followUp.active": true,
    ...
    await sendEmail(admin.email, `Follow-up Reminder — ${overdueLeads.length} overdue, ${dueTodayLeads.length} due today`, html);
...
// Export for manual/test trigger too
