...
const forgotPassword = async (req, res) => {
    try {
        ...
        await (0, emailService_1.sendEmail)(admin.email, "Password Reset Request", `<p>Click the link to reset your password (valid 15 mins):</p><a href="${resetLink}">${resetLink}</a>`);
...
const adminExportMonthlyReport = async (req, res) => {
    try {
        const month = Number(req.query.month);
        const year = Number(req.query.year);
        if (!month || !year) {
        ...
        for (const { key, label } of statusOrder) {
            const groupLeads = leads.filter((l) => (l.status ?? "new") === key);
...
exports.adminExportMonthlyReport = adminExportMonthlyReport;
