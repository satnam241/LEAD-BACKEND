    ...
    try {
        ...
        for (const entry of entries) {
            for (const change of entry?.changes || []) {
                ...
                // Auto message to new Facebook lead (non-blocking)
                (async () => {
                    try {
                        await (0, messageService_1.sendMessageToLead)({
                            leadId: newLead._id.toString(),
                        });
                        console.log("📩 Auto message sent to new FB lead:", String(newLead._id));
                    }
                    catch (err) {
                        console.error("⚠️ Auto message failed for FB lead:", err?.message || err);
                    }
                })();
            }
        }
